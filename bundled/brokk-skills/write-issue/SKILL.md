...
These are free-text inputs. Ask the user in plain text and **stop and wait for their reply** before proceeding.

### If a title is provided as argument (e.g. `/brokk-write-issue Add rate limiting`)
...
2. If the `task` tool is available, spawn an `issue-enhancer`
   agent, passing it the draft title and description. The agent is read-only and does not need an isolated copy of the repo. The agent will use Brokk MCP
   tools to find relevant source code and produce an enhanced issue
   body.

   If the `task` tool is not available, perform the enhancement yourself:
...
Present the labels as a list and **stop and wait for the user's reply**.
