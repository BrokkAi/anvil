...
description: >-
  ...
  get_git_log, get_commit_diff, and analyze_git_hotspots tools, with run_shell_command git/gh fallbacks for blame and GitHub lookups.
---
...
the built-in `run_shell_command` tool with `git` and `gh`.
...
- `search_git_commit_messages` matches the message text; for changes to
  the code itself use `run_shell_command` `git log -S` (literal count changes) or
