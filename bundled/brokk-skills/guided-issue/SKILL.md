...
### If an issue number is provided as argument (e.g. `/brokk-guided-issue 123`)
...
3. If the `task` tool is available, spawn an `issue-diagnostician`
   agent, passing it the full issue text (title, body, comments, labels).
   If the `task` tool is not available, perform the diagnostic analysis yourself using Brokk MCP tools.

4. Present the diagnosis to the user.

5. Present this numbered list and **stop and wait for the user's reply**:
...
1. If the `task` tool is available, spawn an `issue-planner` subagent,
   passing it the diagnosis from Step 2 and the original issue details.
   If the `task` tool is not available, produce the implementation plan yourself using Brokk MCP tools.

2. Present the plan to the user.

3. Present this numbered list and **stop and wait for the user's reply**:
...
4. Execute each step of the plan in order, using `write_file`, `edit`, and `run_shell_command`
...
4. If the `task` tool is available, spawn all 5 reviewer agents in a
   **single response** using parallel `task` tool calls. Use the agent
   names listed below as `subagent_type`.

   If the `task` tool is not available, execute each reviewer's analysis yourself sequentially.
...
For each CRITICAL or HIGH finding, present this numbered menu and **stop and wait for the user's reply**:
...
2. Commit with a sanitized issue title. Compute the sanitized title in
   the same `run_shell_command` invocation as the commit (variables do not persist
   across tool calls):
   ```bash
