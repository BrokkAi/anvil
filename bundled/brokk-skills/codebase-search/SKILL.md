...
- `search_file_contents` takes an array of regex patterns plus an
  ...
  and `context_lines`. Prefer it over the built-in `grep_search` tool: it
...
- `get_summaries` takes project-relative paths or globs like `src/**/*.rs` and returns a compact outline when you want structure, not just file names.
