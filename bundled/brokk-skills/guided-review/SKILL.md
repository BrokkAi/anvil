...
### If a PR number is provided as argument (e.g. `/brokk-guided-review 123`)

Skip directly to **Mode: Remote PR** below using that number.

### If no argument is provided

Present a numbered menu with these options and wait for the user to choose.
...
If the `task` tool is available, spawn all specialist reviewers in a
**single response** using parallel `task` tool calls. Use the agent names
listed below as the `subagent_type`.

If the `task` tool is not available, execute each reviewer's analysis yourself sequentially. For each reviewer, adopt its perspective and use Brokk MCP tools as instructed.
...
Then present this numbered navigation menu and **stop and wait for the user's reply**:
...
- If the finding references a file: use `get_file_contents` for raw
  contents, or `get_summaries` for a declaration-level overview
...
choices from recommendation steps. Present this lettered list and **stop and wait for the user's reply**:
...
3. Apply the recommended edits using `edit`/`write_file` tools. If the finding
...
4. Present this numbered list and **stop and wait for the user's reply**:
