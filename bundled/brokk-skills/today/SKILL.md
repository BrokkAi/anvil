...
Present a numbered menu with these options
:
...
Present this numbered list and **stop and wait for the user's reply** before proceeding:
...
After displaying the list, ask the user what they want to do with these options:
...
When the user wants to write a new issue, activate the `brokk-write-issue` skill with the user's draft if you can continue in the same turn. Otherwise perform the write-issue workflow inline:
...
3. If the `task` tool is available, spawn an `issue-enhancer`
   agent with the draft. Otherwise, use Brokk MCP tools yourself to
...
| Navigate to a PR | Open one in the browser, view unresolved threads, or run `/brokk-review-pr` on it |
...
2. **Navigate to a PR** -- Open in browser / show unresolved threads / run `/brokk-review-pr`
...
which navigation action with these options:

| label | description |
|---|---|
| Open in browser | Run `gh pr view <n> --web` |
| Show unresolved review threads | Print only unresolved threads via jq |
| Run /brokk-review-pr on it | Invoke the `brokk-review-pr` skill on this PR |
...
- **Show unresolved review threads**:
  ...
  ```
- **Run /brokk-review-pr on it**: Activate the `brokk-review-pr` skill with the PR number as the argument if you can continue in the same turn. Otherwise tell the user to run `/brokk-review-pr <number>` themselves.
...
Offer the union of issue and PR actions with these options:

| label | description |
|---|---|
| Select items | Enter numbers from the combined list to include in today's plan |
| Navigate to a PR | Open in browser / show unresolved threads / run `/brokk-review-pr` |
