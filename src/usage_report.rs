...
pub(crate) fn usage_by_model_meta(
    usage_by_model: &BTreeMap<String, anvil_client::llm_client::TokenUsage>,
...
pub(crate) fn insert_bedrock_credits_meta(
    meta: &mut serde_json::Map<String, serde_json::Value>,
    model_wire_id: &str,
    status: anvil_client::bedrock_credits::Status,
) {
...
pub(crate) fn attach_bedrock_credits_meta<F>(
    ...
    F: FnOnce() -> anvil_client::bedrock_credits::Status,
{
    if is_bedrock_model(model_wire_id) {
...
pub(crate) fn insert_openrouter_balance_meta(
    meta: &mut serde_json::Map<String, serde_json::Value>,
    model_wire_id: &str,
    status: anvil_client::openrouter_credits::Status,
) {
...
pub(crate) fn attach_openrouter_balance_meta<F>(
    ...
    F: FnOnce() -> anvil_client::openrouter_credits::Status,
{
    if is_openrouter_model(model_wire_id) {
...
pub(crate) fn insert_deepseek_balance_meta(
    meta: &mut serde_json::Map<String, serde_json::Value>,
    model_wire_id: &str,
    status: anvil_client::deepseek_balance::Status,
) {
...
pub(crate) fn attach_deepseek_balance_meta<F>(
    ...
    F: FnOnce() -> anvil_client::deepseek_balance::Status,
{
    if is_deepseek_model(model_wire_id) {
...
pub(crate) fn estimate_usage_by_model_cost(
    metadata: &[anvil_client::llm_client::ModelMetadata],
    usage_by_model: &BTreeMap<String, anvil_client::llm_client::TokenUsage>,
) -> Option<f64> {
    usage_by_model
        .iter()
...
pub(crate) enum OpenRouterCreditsOutcome {
    ...
    Skipped,
    Fetched(anvil_client::openrouter_credits::Credits),
    Failed(String),
}
...
pub(crate) enum CodexCreditsOutcome {
    ...
    ApiKeyMode,
    Fetched(anvil_client::codex_credits::CodexUsage),
    Failed(String),
}
...
pub(crate) async fn fetch_openrouter_credits_for_usage(
    ...
    let Some(key) = anvil_client::openrouter_credits::active_api_key() else {
        return OpenRouterCreditsOutcome::Skipped;
    };
    match tokio::time::timeout(
        USAGE_CREDITS_FETCH_TIMEOUT,
        anvil_client::openrouter_credits::fetch(&key),
    )
    .await
    {
...
pub(crate) async fn fetch_codex_credits_for_usage(model_wire_id: &str) -> CodexCreditsOutcome {
    ...
    match anvil_client::codex_credits::auth_status() {
        Ok(anvil_client::codex_credits::AuthStatus::Missing) => return CodexCreditsOutcome::NoAuth,
        Ok(anvil_client::codex_credits::AuthStatus::ApiKeyMode) => {
            return CodexCreditsOutcome::ApiKeyMode;
        }
        Ok(anvil_client::codex_credits::AuthStatus::ChatGptMode) => {}
        Err(e) => return CodexCreditsOutcome::Failed(format!("{e:#}")),
    }
    match tokio::time::timeout(
        USAGE_CREDITS_FETCH_TIMEOUT,
        anvil_client::codex_credits::fetch(),
    )
    .await
    {
...
pub(crate) fn render_usage_report(
    ...
    usage: anvil_client::llm_client::TokenUsage,
    cost_usd: Option<f64>,
...
mod tests {
    ...
    fn render_usage_report_with_openrouter_balance_shows_all_lines() {
        ...
        let usage = anvil_client::llm_client::TokenUsage {
            input_tokens: 1_000,
            output_tokens: 500,
            thought_tokens: 200,
        ...
        let credits = anvil_client::openrouter_credits::Credits {
            total_credits: 50.0,
            total_usage: 7.5,
        };
    ...
    fn render_usage_report_shows_codex_metered_credits() {
        let snap = usage_snapshot("codex::gpt-5-codex");
        let usage = anvil_client::codex_credits::CodexUsage {
            plan_type: Some("plus".to_string()),
            credits: Some(anvil_client::codex_credits::CreditStatus {
                has_credits: true,
                unlimited: false,
                balance: Some("12.50".to_string()),
            }),
            rate_limit: Some(anvil_client::codex_credits::RateLimitStatus {
                primary_window: Some(anvil_client::codex_credits::RateLimitWindow {
                    used_percent: 42,
    ...
    fn usage_by_model_keeps_model_attribution_for_cost_and_acp_metadata() {
        let flash_usage = anvil_client::llm_client::TokenUsage {
            input_tokens: 100,
            output_tokens: 10,
            thought_tokens: 2,
        ...
        let pro_usage = anvil_client::llm_client::TokenUsage {
            input_tokens: 200,
            output_tokens: 30,
            thought_tokens: 4,
    ...
    fn bedrock_credit_metadata_attaches_and_preserves_existing_anvil_metadata() {
        ...
        insert_bedrock_credits_meta(
            &mut meta,
            "bedrock::model",
            anvil_client::bedrock_credits::Status::Unavailable {
    ...
    fn openrouter_balance_metadata_attaches_and_preserves_existing_anvil_metadata() {
        ...
        insert_openrouter_balance_meta(
            ...
            anvil_client::openrouter_credits::Status::Available {
                remaining_usd: 0.0,
    ...
    fn openrouter_balance_metadata_is_lazy_for_other_providers() {
        ...
        attach_openrouter_balance_meta(&mut meta, "bedrock::model", || {
            looked_up = true;
            anvil_client::openrouter_credits::Status::Unavailable {
                reason: "refresh pending".into(),
                as_of: "2026-07-15T18:42:00Z".into(),
            }
        });
    ...
    fn deepseek_balance_metadata_attaches_and_preserves_existing_anvil_metadata() {
        ...
        insert_deepseek_balance_meta(
            ...
            anvil_client::deepseek_balance::Status::Available {
                balances: vec![anvil_client::deepseek_balance::Balance {
                    currency: "CNY".into(),
    ...
    fn deepseek_balance_metadata_is_lazy_for_other_providers() {
        ...
        attach_deepseek_balance_meta(&mut meta, "openrouter::model", || {
            looked_up = true;
            anvil_client::deepseek_balance::Status::Unavailable {
                reason: "refresh pending".into(),
                as_of: "2026-07-15T18:42:00Z".into(),
            }
        });
