...
fn structured_output_result(meta: Option<&Meta>) -> Result<Option<StructuredOutputResult>> {
    let Some(payload) = meta
        .and_then(|meta| meta.get(anvil_client::structured_output::ACP_META_NAMESPACE))
...
fn turn_failure_message(meta: Option<&Meta>) -> Option<String> {
    meta?
        .get(anvil_client::structured_output::ACP_META_NAMESPACE)?
        .get("turnFailure")?
        .get("message")?
        .as_str()
        .map(str::to_string)
...
mod tests {
    ...
    fn response_schema_round_trips_through_prompt_and_result_metadata() {
        ...
        let parsed = anvil_client::structured_output::parse_structured_output_request(Some(&meta))
            .unwrap()
            .unwrap();
