...
pub(crate) async fn run(
    ...
    'outer: for turn in 0..turn_limit {
            ...
            .filter(|_| p2t_steps_executed == 0)
        {
            ...
            shell_sandbox_retry_states = outcome.shell_sandbox_retry_states;
...
async fn execute_step_tool_calls(
    ...
    let mut available_shell_sandbox_retry_states = shell_sandbox_retry_states.to_vec();
    ...
    for state in next_shell_sandbox_retry_states {
        push_unique_shell_sandbox_retry_state(&mut available_shell_sandbox_retry_states, state);
    }
...
const SANDBOX_FAILURE_ESCALATION_HINT: &str = "\n\n⚠️  This command appears to be blocked by the sandbox boundary. The next `run_shell_command` schema now permits a one-time outside-sandbox retry of this exact command, unchanged, in the same directory. Retry it with `sandbox_permissions: \"require_escalated\"` rather than asking the user to paste in external content when the sandbox is the blocker (for example blocked network, DNS, or filesystem access).";
...
async fn execute_subagent(
    ...
    // closing line -- the reason has to be reported here. A turn-limit or
    // cancellation exit is named explicitly rather than collapsed into the
    // generic "empty response" message it used to share with a model that
    // simply said nothing.
    //
    // The empty-vs-real decision keys on `had_text` from the `LoopStop`, NOT on
    // `text.trim().is_empty()`: `run` appends its closing notice to the returned
    // text for the silent terminations, so an emptiness check would mistake that
    // notice for a real answer. `had_text` is captured before the notice is
    // appended, so it still reflects whether the model itself produced output.
    let (text, _exchanges, _replay_events, nested_usage, stop) = nested;
    let failed_output = match &stop {
