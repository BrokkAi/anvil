...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum SourceRelationshipGrounding {
    ExplicitTraceIntent,
    SourceSyntaxShape,
    SameTokenBifrostHit,
    DeterministicRepairContext,
    PriorExactSourceContext,
    IdentifierShapeOnly,
    None,
    Unknown,
}

impl Default for SourceRelationshipGrounding {
    fn default() -> Self {
        Self::Unknown
    }
}
...
impl Default for ExactnessRole {
    fn default() -> Self {
        Self::Unknown
    }
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum TextAllowReason {
    None,
    PostEditVerification,
    ExactLiteralSurface,
    ArtifactOrOutputText,
    ExternalApiSurfaceText,
    ImportIncludePackageText,
    InsufficientSymbolGrounding,
    Unknown,
}

impl Default for TextAllowReason {
    fn default() -> Self {
        Self::None
    }
}
...
pub struct RawGateClassifierOutput {
    ...
    #[serde(default)]
    pub source_relationship_grounding: SourceRelationshipGrounding,
    #[serde(default)]
    pub exactness_role: ExactnessRole,
    #[serde(default)]
    pub text_allow_reason: TextAllowReason,
    #[serde(default)]
...
fn normalize_raw_text_classifier_consistency(raw: &mut RawGateClassifierOutput) {
    ...
    if raw.allow_exception == TextAllowException::NonSourceText
        && raw.object_of_search == ObjectOfSearch::SourceSymbolRelationship
    {
        raw.allow_exception = TextAllowException::None;
    }
    if has_same_direct_candidate
        && raw.evidence.symbol_tokens.len() == 1
        && raw.evidence.same_path_recent_bifrost_hit
        ...
        && !matches!(
            ...
            TextPatternClass::ExternalApiOrAnnotationNames
                ...
                | TextPatternClass::PathLike
                | TextPatternClass::NaturalLanguage
        )
    {
        raw.material_constraints.clear();
        raw.allow_exception = TextAllowException::None;
    }
    if has_same_direct_candidate
...
fn source_relationship_grounded(raw: &RawGateClassifierOutput) -> bool {
    match raw.source_relationship_grounding {
        ...
        | SourceRelationshipGrounding::SourceSyntaxShape
        | SourceRelationshipGrounding::SameTokenBifrostHit
        | SourceRelationshipGrounding::DeterministicRepairContext
        | SourceRelationshipGrounding::PriorExactSourceContext => return true,
        SourceRelationshipGrounding::IdentifierShapeOnly | SourceRelationshipGrounding::None => {
            return false;
        }
        SourceRelationshipGrounding::Unknown => {}
    }
    if matches!(
        ...
        RelationshipKind::CallSite
            ...
            | RelationshipKind::BroadSymbolOrientation
    ) && raw.pattern_class != TextPatternClass::IdentifierLike
    {
        return true;
    }
    matches!(
    ...
    "json_schema": {{
      ...
      "schema": {{
        ...
        "properties": {{
          ...
          "source_relationship_grounding": {{"type":"string","enum":["explicit_trace_intent","source_syntax_shape","same_token_bifrost_hit","deterministic_repair_context","prior_exact_source_context","identifier_shape_only","none","unknown"]}},
          "exactness_role": {{"type":"string","enum":["target_characters","source_relationship_filter","post_edit_verification","unknown"]}},
          ...
          "evidence": {{
            ...
            "properties": {{
              ...
              "same_token_bifrost_hit": {{"type":"boolean"}},
              "same_token_bifrost_miss": {{"type":"boolean"}},
              "same_token_or_path_bifrost_miss": {{"type":"boolean"}},
              "same_path_recent_edit_or_write": {{"type":"boolean"}},
              "same_path_bifrost_hit": {{"type":"boolean"}},
              "related_symbol_bifrost_hit": {{"type":"boolean"}},
              "same_path_recent_bifrost_hit": {{"type":"boolean"}},
              "exact_text_or_regex_needed": {{"type":"boolean"}}
            }},
            "required": ["symbol_tokens","same_token_bifrost_hit","same_token_bifrost_miss","same_token_or_path_bifrost_miss","same_path_recent_edit_or_write","same_path_bifrost_hit","related_symbol_bifrost_hit","same_path_recent_bifrost_hit","exact_text_or_regex_needed"]
          }},
          "bifrost_candidate": {{
            "type":["object","null"],
            "additionalProperties": false,
...
fn enforce_text_classifier_policy(output: &mut GateClassifierOutput, context: &GateContext) {
    ...
    let ungrounded_bare_identifier = ungrounded_bare_identifier_probe(
        pattern,
        glob,
        path,
        file_path,
    ...
    if same_entity_intersection_regex(pattern) {
        output.decision = GateClassifierDecision::AllowText;
        output.recommended_tool = RecommendedTool::None;
        output.suggested_args = json!({});
        output.reason = format!(
            "ALLOW_TEXT because `{}` requires same-entity source predicate matching that Bifrost cannot preserve in one tool call.",
        ...
        && bifrost_navigation_scope(glob, path, file_path)
    {
        let (tool, relation) = repair_tool
            .as_ref()
            .map(|(tool, relation)| (tool.clone(), *relation))
            .unwrap_or((RecommendedTool::SearchSymbols, "declaration"));
    ...
    if (repair_tool.is_none()
        ...
        || text_like_bare_identifier_scope(pattern, glob, path, file_path)
        || ungrounded_bare_identifier
        || (repair_tool.is_none()
            && !same_direct_bifrost_candidate(output)
...
fn ungrounded_bare_identifier_probe(
    ...
    file_path: &str,
    context: &GateContext,
    has_repair_tool: bool,
) -> bool {
    if has_repair_tool || !bifrost_navigation_scope(glob, path, file_path) {
        return false;
    }
    if regex_wrapped_single_identifier(pattern).is_none() {
        return false;
    }
    if same_symbol_bifrost_hit_for_grep(pattern, &context.tool_exchanges) {
        return false;
    }
    !(declaration_search_pattern(pattern)
...
fn text_like_bare_identifier_scope(pattern: &str, glob: &str, path: &str, file_path: &str) -> bool {
    regex_wrapped_single_identifier(pattern).is_some()
...
fn bifrost_candidate_has_args(output: &GateClassifierOutput) -> bool {
    output.bifrost_candidate.as_ref().is_some_and(|candidate| {
        candidate
            .args
            .as_object()
...
fn source_navigation_text_intent(intent: &TextIntent) -> bool {
    matches!(
        intent,
...
fn declaration_repair_compatible_intent(intent: &TextIntent) -> bool {
    source_navigation_text_intent(intent) || matches!(intent, TextIntent::Unknown)
}
...
fn repair_blocked_text_grep(
    ...
    must_allow_text_grep(pattern, glob, path, file_path)
        ...
        || recent_naming_variant_text_search(
            pattern,
            glob,
            path,
            file_path,
            &context.tool_exchanges,
        )
...
fn same_entity_intersection_regex(pattern: &str) -> bool {
    if !declaration_search_pattern(pattern) {
        return false;
    }
    let Some(after_boundary) = declaration_body_or_rhs_suffix(pattern) else {
        return false;
    };
    let concrete_suffix_tokens: Vec<_> = identifier_tokens(after_boundary)
        .into_iter()
        .filter(|token| same_entity_suffix_token(token))
        .collect();
    !concrete_suffix_tokens.is_empty()
}

fn declaration_shape_without_material_intersection(pattern: &str) -> bool {
    declaration_search_pattern(pattern) && !same_entity_intersection_regex(pattern)
}

fn declaration_body_or_rhs_suffix(pattern: &str) -> Option<&str> {
    for boundary in ["=>", "=", "\\{", "{"] {
        if let Some((_, suffix)) = pattern.split_once(boundary) {
            return Some(suffix);
        }
    }
    None
...
fn same_entity_suffix_token(token: &str) -> bool {
    if token.len() < 3 {
        return false;
    }
    let lower = token.to_ascii_lowercase();
...
fn compact_classifier_evidence(tool_name: &str, args: &Value, exchanges: &[ToolExchange]) -> Value {
    ...
    json!({
        ...
        "same_token_bifrost_hit": symbols.iter().any(|token| overlaps_recent_bifrost_hit(token, exchanges)),
        ...
        "same_path_bifrost_hit": overlaps_recent_bifrost_hit(&target, exchanges),
        "same_symbol_bifrost_hit": symbols.iter().any(|token| overlaps_recent_bifrost_hit(token, exchanges)),
        "related_symbol_bifrost_hit": symbols.iter().any(|token| overlaps_recent_bifrost_hit(token, exchanges)),
...
fn identifier_tokens(pattern: &str) -> Vec<String> {
    let normalized = pattern.replace("\\b", " ");
    normalized
...
fn script_execution_semantics(command: &str) -> bool {
    let lower = command.to_ascii_lowercase();
    if !scriptish(&lower) {
        return false;
    }
    lower.contains("runpy.")
        || lower.contains("run_module(")
        || lower.contains("run_path(")
        || lower.contains("subprocess.")
        || lower.contains("assert ")
        || lower.contains("unittest")
        || lower.contains("pytest")
        || lower.contains("redirect_stdout")
        || lower.contains("sys.modules")
    ...
    command.split([';', '\n']).any(|segment| {
        ...
        parts.iter().enumerate().skip(1).any(|(index, part)| {
            ...
            let upstream = parts[..index]
                ...
                .unwrap_or_default();
            !matches!(upstream.as_str(), "cat" | "head" | "tail" | "sed" | "nl")
        })
    })
}
...
mod tests {
    ...
    fn text_output_json(
        ...
        json!({
            ...
            "source_relationship_grounding": if gate { "explicit_trace_intent" } else { "none" },
            "exactness_role": if gate { "source_relationship_filter" } else { "target_characters" },
            "text_allow_reason": if gate { "none" } else { "exact_literal_surface" },
    ...
    fn classifier_prompt_frames_flash_as_advisory() {
        ...
        assert!(body.contains("Do not include an action or decision field"));
        assert!(body.contains("source_relationship_grounding"));
        assert!(body.contains("exactness_role"));
        assert!(body.contains("source_relationship_filter"));
    ...
    #[test]
    fn source_relationship_non_source_text_contradiction_routes_to_bifrost() {
        let envelope = json!({
            "choices": [{
                "message": {
                    "content": json!({
                        "reason": "Source declaration regex over source scope should use Bifrost.",
                        "intent": "symbol_definition_lookup",
                        "pattern_class": "symbol_glob",
                        ...
                        "source_relationship_grounding": "explicit_trace_intent",
                        "exactness_role": "source_relationship_filter",
                        "text_allow_reason": "none",
                        "material_constraints": [],
                        "bifrost_preserves_material_constraints": "no",
                        "bifrost_capability_needed": ["references"],
                        "prior_bifrost_status": "observed_hit",
                        "fallback_exception": "non_source_text",
                        "evidence": {
                            "symbol_tokens": ["case", "class"],
                            "same_token_or_path_bifrost_miss": false,
                            "same_path_recent_edit_or_write": false,
                            "same_path_recent_bifrost_hit": false,
                            "exact_text_or_regex_needed": false
                        },
                        "bifrost_candidate": {"tool": "search_symbols", "args": {"patterns":["case class"]}},
                        "suggested_args": {},
                        "confidence": "high"
                    }).to_string()
                }
    ...
    #[test]
    fn single_source_identifier_with_recent_bifrost_hit_ignores_hallucinated_constraints() {
        let envelope = json!({
            "choices": [{
                "message": {
                    "content": json!({
                        "reason": "Known source identifier has same-direct Bifrost candidate.",
                        "intent": "symbol_usage_lookup",
                        "pattern_class": "symbol_glob",
                        "scope_class": "repository_wide",
                        "bifrost_fit": "same_or_more_direct",
                        "object_of_search": "source_symbol_relationship",
                        "relationship_kind": "usage",
                        "material_constraints": ["exact_argument", "qualifier", "generic_signature", "assignment_operator"],
                        "bifrost_preserves_material_constraints": "lossy_superset",
                        "bifrost_capability_needed": ["references"],
                        "prior_bifrost_status": "observed_hit",
                        "fallback_exception": "none",
                        "evidence": {
                            "symbol_tokens": ["x_mitre_permissions_required"],
                            "same_token_or_path_bifrost_miss": false,
                            "same_path_recent_edit_or_write": false,
                            "same_path_recent_bifrost_hit": true,
                            "exact_text_or_regex_needed": false
                        },
                        "bifrost_candidate": {"tool": "scan_usages", "args": {"pattern":"x_mitre_permissions_required"}},
                        "suggested_args": {},
                        "confidence": "high"
                    }).to_string()
                }
    ...
    #[test]
    fn non_empty_candidate_args_preserve_explicit_tool_despite_coarse_intent() {
        let mut output = GateClassifierOutput {
            reason: "The candidate names exact symbols to search.".to_string(),
            ...
            bifrost_candidate: Some(BifrostCandidate {
                tool: Some(RecommendedTool::SearchSymbols),
                args: json!({"patterns":["LOCUnknown", "UnknownTitle", "Unknown"], "include_tests": false}),
            }),
            decision: GateClassifierDecision::AllowText,
            recommended_tool: RecommendedTool::None,
            suggested_args: json!({}),
    ...
    #[test]
    fn ungrounded_bare_identifier_probe_stays_text() {
        for pattern in ["effectAsyncM", "\\beffectAsyncM\\b"] {
            let mut output = GateClassifierOutput {
                reason: "Classifier claims this bare identifier is a known symbol.".to_string(),
                intent: TextIntent::SymbolDefinitionLookup,
                pattern_class: TextPatternClass::SymbolGlob,
                scope_class: TextScopeClass::RepositoryWide,
                bifrost_fit: BifrostFit::SameOrMoreDirect,
                ...
                bifrost_candidate: Some(BifrostCandidate {
                    tool: Some(RecommendedTool::SearchSymbols),
                    args: json!({"patterns":["effectAsyncM"]}),
                }),
                decision: GateClassifierDecision::GateToSymbolTool,
                recommended_tool: RecommendedTool::SearchSymbols,
                suggested_args: json!({"patterns":["effectAsyncM"]}),
                confidence: GateConfidence::High,
            };
            let mut prior = exchange("search_symbols");
            prior.arguments = r#"{"patterns":["TaskExecutor"]}"#.to_string();
            prior.result = "TaskExecutor found in src/TaskExecutor.scala".to_string();
            let context = GateContext {
                tool_name: "grep_search".to_string(),
                args: json!({"pattern": pattern, "path": ".", "glob": "*.scala"}),
                messages: Vec::new(),
                tools: Vec::new(),
    ...
    #[test]
    fn xaml_binding_identifier_stays_text() {
        let mut output = GateClassifierOutput {
            reason: "Classifier claims this XAML binding token is symbol navigation.".to_string(),
            intent: TextIntent::SymbolDefinitionLookup,
            pattern_class: TextPatternClass::SymbolGlob,
            scope_class: TextScopeClass::BroadSourceScope,
            bifrost_fit: BifrostFit::SameOrMoreDirect,
            allow_exception: TextAllowException::None,
            evidence: TextEvidence {
                symbol_tokens: vec!["ToggleFilterPanelCommand".to_string()],
                same_token_or_path_bifrost_miss: false,
                same_path_recent_edit_or_write: false,
                same_path_recent_bifrost_hit: false,
                exact_text_or_regex_needed: false,
            ...
            bifrost_candidate: Some(BifrostCandidate {
                tool: Some(RecommendedTool::SearchSymbols),
                args: json!({"symbols":["ToggleFilterPanelCommand"]}),
            }),
            decision: GateClassifierDecision::GateToSymbolTool,
            recommended_tool: RecommendedTool::SearchSymbols,
            suggested_args: json!({"symbols":["ToggleFilterPanelCommand"]}),
            confidence: GateConfidence::High,
        };
        let context = GateContext {
            tool_name: "grep_search".to_string(),
            args: json!({"pattern": "ToggleFilterPanelCommand", "path": "source", "glob": "*.xaml"}),
            messages: Vec::new(),
            tools: Vec::new(),
            tool_exchanges: Vec::new(),
        };
    ...
    #[test]
    fn same_token_usage_candidate_survives_same_path_hit() {
        let mut output = GateClassifierOutput {
            reason: "Known symbol reference search should use scan_usages.".to_string(),
            intent: TextIntent::SymbolUsageLookup,
            pattern_class: TextPatternClass::SymbolGlob,
            scope_class: TextScopeClass::BroadSourceScope,
            bifrost_fit: BifrostFit::SameOrMoreDirect,
            allow_exception: TextAllowException::None,
            evidence: TextEvidence {
                symbol_tokens: vec!["absVal".to_string()],
                same_token_or_path_bifrost_miss: false,
                same_path_recent_edit_or_write: true,
                same_path_recent_bifrost_hit: true,
                exact_text_or_regex_needed: false,
            },
            bifrost_candidate: Some(BifrostCandidate {
                tool: Some(RecommendedTool::ScanUsages),
                args: json!({"symbols":["absVal"],"include_tests":true}),
            }),
            decision: GateClassifierDecision::GateToSymbolTool,
            recommended_tool: RecommendedTool::ScanUsages,
            suggested_args: json!({"symbols":["absVal"],"include_tests":true}),
            confidence: GateConfidence::High,
        };
        let mut prior = exchange("search_symbols");
        prior.arguments = r#"{"patterns":["absVal"]}"#.to_string();
        prior.result = "absVal found in Maths/Abs.js".to_string();
        let context = GateContext {
            tool_name: "grep_search".to_string(),
            args: json!({"pattern": "\\babsVal\\b", "path": ".", "glob": "*.js"}),
            messages: Vec::new(),
            tools: Vec::new(),
            tool_exchanges: vec![prior],
        };
    ...
    #[test]
    fn word_boundary_identifier_tokens_drop_regex_boundary_marker() {
        assert_eq!(identifier_tokens("\\babsVal\\b"), vec!["absVal"]);
    }
    ...
    fn recent_naming_variant_grep_stays_text() {
        let mut output = GateClassifierOutput {
            reason: "GATE_TO_SYMBOL_TOOL because property references are source navigation."
                .to_string(),
            intent: TextIntent::SymbolReferenceLookup,
            pattern_class: TextPatternClass::IdentifierLike,
            scope_class: TextScopeClass::BroadSourceScope,
    ...
    fn lower_snake_same_symbol_hit_repairs_to_bifrost() {
        ...
        hit.result =
            r#"{"files":[{"functions":[{"signature":"fn mount_info_from_path"}]}]}"#.to_string();
        let context = GateContext {
            tool_name: "grep_search".to_string(),
    ...
    #[test]
    fn same_entity_intersection_declaration_regex_stays_text() {
        let context = GateContext {
            tool_name: "grep_search".to_string(),
            args: json!({
                "pattern": "def .*: String = .*getOrElse",
                "glob": "thehive/app/org/thp/thehive/models/*.scala",
                "path": "."
            }),
            messages: Vec::new(),
            tools: Vec::new(),
        ...
        let mut output = GateClassifierOutput {
            reason: "GATE_TO_SYMBOL_TOOL because this is declaration-shaped.".to_string(),
            intent: TextIntent::SymbolDefinitionLookup,
            pattern_class: TextPatternClass::SymbolGlob,
            scope_class: TextScopeClass::BroadSourceScope,
            bifrost_fit: BifrostFit::SameOrMoreDirect,
            allow_exception: TextAllowException::None,
            evidence: TextEvidence {
                symbol_tokens: vec!["String".to_string(), "getOrElse".to_string()],
                ..TextEvidence::default()
            },
            bifrost_candidate: Some(BifrostCandidate {
                tool: Some(RecommendedTool::SearchSymbols),
                args: json!({"patterns":["def .*: String = .*getOrElse"],"include_tests":false}),
            }),
            decision: GateClassifierDecision::GateToSymbolTool,
            recommended_tool: RecommendedTool::SearchSymbols,
            suggested_args: json!({}),
    ...
    #[test]
    fn broad_declaration_default_shape_still_routes_to_symbols() {
        let context = GateContext {
            tool_name: "grep_search".to_string(),
            args: json!({
                "pattern": "case class .*\\(.*= .*\\)",
        ...
        let mut output = GateClassifierOutput {
            reason: "ALLOW_TEXT because the regex default-value syntax is exact.".to_string(),
            intent: TextIntent::Unknown,
            pattern_class: TextPatternClass::RegexText,
            ...
            allow_exception: TextAllowException::None,
            evidence: TextEvidence {
                symbol_tokens: vec!["case".to_string(), "class".to_string()],
                same_path_recent_bifrost_hit: true,
                exact_text_or_regex_needed: true,
                ..TextEvidence::default()
            },
