...
| `lib/registry.mjs` | Registry visibility checks for the native package publisher. Reads with `--prefer-online` and waits past npmjs' five-minute packument cache, so a just-published version is never mistaken for a failed publish. |
