...
- **ACP session config options are session-scoped only.** Add `SessionConfigOption` ids only for client-visible session controls such as behavior mode, permission mode, model selection, or reasoning effort. Do not put host/UI/install-wide preferences in `all_config_options`, `CONFIGURE_KNOWN_KEYS`, or `apply_config_option`; use `/setup`, `SetupState`, or a dedicated config file instead.
