...
def score_study(
    ...
    for row in continuations:
        ...
        _add_usage(continuation_usage, row.get("usage"))
    ...
    final_top_k = final_ranking[:top_k] if valid_top_k else []
    recalled = len(set(survivors) & set(final_top_k))
    top_k_recall = recalled / top_k if valid_top_k else None
    final_winner = final_ranking[0] if final_ranking else None
    final_winner_survived = final_winner in survivors if final_winner is not None else None
    eligible_for_gate = (
...
def summarize(
    ...
    recalled = sum(study["top_k_recalled"] for study in eligible)
    opportunities = sum(study["top_k"] for study in eligible)
    winner_survivals = sum(study["final_winner_survived"] is True for study in eligible)
    observed = recalled / opportunities if opportunities else None
    gate_status = "insufficient-data"
    if len(eligible) >= min_complete_studies:
    ...
    return {
        ...
        "summary": {
            ...
            "top2_recalled_slots": recalled,
            "top2_opportunities": opportunities,
            "top2_recall": observed,
            "top2_recall_wilson_95": _wilson(recalled, opportunities),
            "final_winner_survival": (
                winner_survivals / len(eligible) if eligible else None
            ),
